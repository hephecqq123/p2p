...
function jinjichange(v) {
    var str = "0";
    var j = ["j1", "j2", "j3"];
    for (i = 0; i < v; i++) { str += ("_" + (document.getElementById(j[i]).selectedIndex - 1));}
    ;
    var ss = document.getElementById(j[v]);
    with (ss) {
        length = 0;
        options[0] = new Option(opt0[v], opt0[v]);
        if (v && document.getElementById(j[v - 1]).selectedIndex > 0 || !v) {
            if (dsy.Exists(str)) {
                ar = dsy.Items[str];
                for (i = 0; i < ar.length; i++)options[length] = new Option(ar[i], ar[i]);
                if (v)options[0].selected = true;
...
function setup() {
    ...
    document.getElementById(s[s.length - 1]).onchange = new Function("promptinfo();");
    change(0);
};
function setotherup() {
    for (i = 0; i < j.length - 1; i++)
        document.getElementById(j[i]).onchange = new Function("jinjichange(" + (i + 1) + ");");
    document.getElementById(j[j.length - 1]).onchange = new Function("jinjiinfo();");
    jinjichange(0);
}
